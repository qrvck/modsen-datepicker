...
function createDate(date: Date, firstDayOfWeek: 'sunday' | 'monday' = 'monday') {
  ...
  const dayName = date.toLocaleDateString('en-US', { weekday: 'long' }); // название дня недели (воскресенье)
  const shortDayName = date.toLocaleDateString('en-US', { weekday: 'short' }); // название дня недели (вс)
  ...
  const monthName = date.toLocaleDateString('en-US', { month: 'long' }); // февраль
  const shortMonthName = date.toLocaleDateString('en-US', { month: 'short' }); // февр.

  const year = date.getFullYear(); // год 2024
  const shortYear = date.toLocaleDateString('en-US', { year: '2-digit' }); // год 24
